#!/usr/bin/env bash
# ==============================================================================
#  Animedekho Bot — Ubuntu x86_64 Automated Setup (Docker Compose)
# ==============================================================================
...
echo -e "${GREEN}======================================================${NC}"
echo -e "${GREEN}    Animedekho Bot — Ubuntu x86 Docker Compose Setup  ${NC}"
echo -e "${GREEN}======================================================${NC}"
...
if [ "$TOTAL_RAM_MB" -ge 4000 ]; then
    ...
    if free | awk '/^Swap:/ {exit !$2}'; then
        ...
        # Calculate swap size: if RAM < 2GB, use 2GB swap; otherwise use 1GB swap
...
if ! command -v docker &> /dev/null; then
    ...
    # Add current user to docker group (for non-root usage after re-login)
    if [ -n "$SUDO_USER" ]; then
        usermod -aG docker "$SUDO_USER"
    fi

    echo -e "${GREEN}[OK] Docker + Docker Compose installed and started!${NC}"
else
...
# ==============================================================================
echo -e "\n${YELLOW}Step 5/5 — Building & Starting with Docker Compose...${NC}"
...
echo -e "  ${YELLOW}cd /opt/animedekho-bot && sudo git pull && sudo docker compose up -d --build${NC}"
